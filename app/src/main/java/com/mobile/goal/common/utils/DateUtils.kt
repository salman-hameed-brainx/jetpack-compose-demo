...
import com.mobile.goal.common.utils.DateConstants.UTC_PATTERN_WITHOUT_ZONE
import com.mobile.goal.common.utils.DateConstants.YYYY_MM_DD_HH_MM_AA
import com.mobile.goal.common.utils.DateConstants.YYYY_MM__DD
import com.mobile.goal.common.utils.DateConstants.hh_mm_aa
import android.annotation.SuppressLint
import android.text.format.DateUtils
import com.mobile.goal.common.utils.DateConstants.DD_MMM
import com.mobile.goal.common.utils.DateConstants.UTC_PATTERN_WITH_ZONE
...
fun getCurrentDateStringFormat(): String? {
    val dateFormat =  SimpleDateFormat(YYYY_MM__DD)
...
@SuppressLint("SimpleDateFormat")
fun getTimeConcatString(dateTime: Date?, hour: String?, mint: String?, dayTime:String, locale: String?="en", dateFormat:String= YYYY_MM_DD_HH_MM_AA, timeFormat:String=hh_mm_aa):Map<String, Any>{
    DateConstants.apply {
        val dateString = dateTime.formatDateIntoString(YYYY_MM__DD,false)
        val time = "$dateString $hour:$mint $dayTime"
        val date = SimpleDateFormat(dateFormat, Locale(locale, "")).parse(
            time
        )
        return mapOf(
            ExtEnums.TIME_STRING.key to date.formatDateIntoString(timeFormat),
...
fun Calendar.formatDateIntoString(dateFormatter:String,shouldAddLocale:Boolean = true,enableTimeZone:Boolean=false,locale: String?="en") : String {
    return time.formatDateIntoString(dateFormatter,shouldAddLocale,enableTimeZone,locale)
}
...
fun isDateInFuture(date1: Calendar): Boolean {
    return date1.timeInMillis > getCalender().timeInMillis
}

fun isDateInPast(date1: Calendar): Boolean {
    return date1.timeInMillis < getCalender().timeInMillis
}


fun isCurrentDate(date1: Calendar?, date2: Calendar? = getCalender()): Boolean {
    if (date1 == null || date2 == null) return false
...
fun Date.isTomorrow(date: Date?):Boolean{
    return DateUtils.isToday(time - DateUtils.DAY_IN_MILLIS)
}

fun getCombineStartEndDate(start:String?,end:String?):String{
    return "${start?.getDateTimeFromDateString(UTC_PATTERN_WITH_ZONE, timeZoneEnable = true)?.formatDateIntoString(DD_MMM)}-${end?.getDateTimeFromDateString(UTC_PATTERN_WITH_ZONE, timeZoneEnable = true)?.formatDateIntoString(DD_MMM)}"
}