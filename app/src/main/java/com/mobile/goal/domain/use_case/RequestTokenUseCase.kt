...
class RequestTokenUseCase @Inject constructor(
    ...
    operator fun invoke(requestTokenDto: RequestTokenDto): Flow<Resource<AccessToken>> = flow {
        try {
            emit(Resource.Loading())
            val accessToken = repository.requestToken(requestTokenDto).toAccessToken()
            sessionDataStore.updateAccessToken(accessToken)
