...
class GetEntrants @Inject constructor(
    private val repository: CompetitionRepository
) {
    operator fun invoke(code: String,distanceId: Int): Flow<Resource<CompetitionParticipantsResponse?>> = flow {
        try {
            emit(Resource.Loading())
            emit(Resource.Success(repository.getCompetitionParticipants(code,distanceId)))
