...
import com.mobile.goal.data.remote.dto.SignInGoogleDto
...
class SignInGoogleUseCase @Inject constructor(
    ...
    operator fun invoke(signInGoogleDto: SignInGoogleDto): Flow<Resource<Code>> = flow {
        try {
            emit(Resource.Loading())
            val code = repository.signInGoogle(signInGoogleDto).toCode()
